<script lang="ts">
  ...
  import { STRATEGIC_NAV, OPERATIONAL_NAV_ADMIN, OPERATIONAL_NAV_CS } from '$lib/config/navigation';
  ...
  const operationalNav = $derived(isAdmin ? OPERATIONAL_NAV_ADMIN : OPERATIONAL_NAV_CS);
</script>
...
<div class="flex h-screen transition-colors duration-300 overflow-hidden {theme.current === 'dark' ? 'dark' : ''}" style:background-color="var(--bg-app)" style:color="var(--text-app)">
  ...
  <aside class="hidden md:flex md:w-64 md:flex-col bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white border-r border-zinc-200 dark:border-white/5 transition-colors">
    <div class="flex flex-col flex-1 min-h-0">
      ...
      <div class="flex items-center h-20 px-6 border-b border-zinc-200 dark:border-white/5">
        ...
        <span class="text-lg font-black tracking-tight text-brand-charcoal dark:text-white">Gourmet<span class="text-brand-primary">{isAdmin ? 'Admin' : 'Hub'}</span></span>
      </div>

      <!-- User Label -->
      <div class="px-6 py-4 border-b border-zinc-200 dark:border-white/5 {isAdmin || theme.current === 'dark' ? 'bg-zinc-50 dark:bg-white/5' : 'bg-zinc-50/50'}">
      ...
      <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        {:else}
        <!-- Navigation for CS only -->
          {#each OPERATIONAL_NAV_CS as item}
            <a
              href={item.href}
              class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all
                     {page.url.pathname === item.href 
  ...
  <div class="flex flex-col flex-1 overflow-hidden">
    <!-- Header -->
    <header class="h-20 transition-colors border-b relative z-20" style:background-color="var(--bg-app)" style:border-color="var(--border-app)">
